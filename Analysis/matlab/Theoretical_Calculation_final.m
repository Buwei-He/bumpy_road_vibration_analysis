...
theta0 =atan((0.879-0.4799051531)/0.5313848963) ; %初始状态下小车质心到Beam转轴的连线与水平方向的夹角 弧度制 check
% adams中小车质心坐标：-9.3360264826E-03, 0.4799051531, 2.4788406839E-02 y↑z→
...
m, 0, 0;
0, J, 0;
mb*lbc, -mb*lbc*D*cos(theta0), (Jb+mb*lbc^2)
];
...
F_Rs = [(cb*hb_dot_s + cf*hf_dot_s + kb*hbs +kf*hfs + m*g/s);
    (-Lb*cb*hb_dot_s + Lf*cf*hf_dot_s - Lb*kb*hbs + Lf*kf*hfs);
    (-mb*g*lbc/s)];

%% Rigid Beam

Lxr=(M_R*s^2+C_R*s+K_R)\F_Rs;

Yc_r = simplify(Lxr(1));
Theta_r = simplify(Lxr(2));
Psi_r = simplify(Lxr(3));

Delta_r = Yc_r + D*sin(theta0)/s - D*cos(theta0)*Theta_r + lf*Psi_r - 0.3990948469/s;

[nR,dR] = numden(Delta_r);
numR = sym2poly(nR);
denR = sym2poly(dR);
figure
impulse(tf(numR,denR))
xlim([0 7])
title("Rigid Beam Impulse Delta_r(t)")

ILxr=ilaplace(Lxr,s,t);
ILxr=vpa(ILxr,200);
yc_r = ILxr(1,1);
theta_r = ILxr(2,1);
psi_r = ILxr(3,1);
delta_r = yc_r + D*(sin(theta0) - cos(theta0)*theta_r)+ lf*psi_r - 0.3990948469;

integrate_r = simplify(int(delta_r^2*v,t,0,33.91593/v))
sum_r = 0;
% subs(delta,t,1)
for i = 0 : 0.01 : 7
    sum_r = sum_r + ((subs(delta_r,t,i)+subs(delta_r,t,i+0.01))/2)^2 * v * 0.01;
end
sum_r
...
xlim([0 7])
title("Dynamic Beam Impulse Y_c(t)")

Delta = Yc + D*sin(theta0)/s - D*cos(theta0)*Theta + lf*Psi - Q - 0.3981506469/s;
[nD,dD] = numden(Delta);
numD = sym2poly(nD);
denD = sym2poly(dD);
figure
impulse(tf(numD,denD))
xlim([0 7])
title("Dynamic Beam Impulse Delta(t)")
...
psi = ILx(3,1);
q = ILx(4,1);
delta = yc + D*(sin(theta0) - cos(theta0)*theta)+ lf*psi- q - 0.3981506469;

integrate_d = simplify(int(delta^2*v,t,0,33.91593/v))
sum_d = 0;
% subs(delta,t,1)
for i = 0 : 0.01 : 7
    sum_d = sum_d + ((subs(delta,t,i)+subs(delta,t,i+0.01))/2)^2 * v * 0.01;
end
sum_d

figure 
fsurf(yc, [0, 7])
title("Dynamic Beam y_c(t)")
view(0, 0)
figure 
fsurf(q, [0, 7])
title("Dynamic Beam q(t)")
view(0, 0)
figure
fsurf(delta, [0, 7])
title("Dynamic Beam delta(t)")
view(0, 0)
